...
function onSubmit(evt) {
    ...
    if (!requestImg) { return Notiflix.Notify.warning('Sorry, there are no images matching your search query. Please try again.')}
...
async function  fetchPictures(query, page) {
    ...
    const {data} = response;
        if (data.hits.length === 0) { Notiflix.Notify.warning(notify) };
    return data;
    } catch (error) {
    console.error(error);
  }
